...
def browse():
    ...
    try:
        ...
        folders = sorted([i["name"] for i in items if i.get("id") is None or i.get("metadata") is None])
    except Exception:
        pass
...
def download():
    ...
    try:
        file_bytes = client.storage.from_(STORAGE_BUCKET).download(f"{folder}/{filename}")
        ...
        if ext == ".pdf":
            ...
            # DOCX — serve directly, generate ID for logging only

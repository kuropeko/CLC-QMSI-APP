...
# ── SUPABASE CONFIG ──────────────────────────────────────────
SUPABASE_URL      = "https://awuqqfurmnyqvkfrtehf.supabase.co"
SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF3dXFxZnVybW55cXZrZnJ0ZWhmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI1ODc4NTgsImV4cCI6MjA4ODE2Mzg1OH0.8A92RyzX22hSUKOJ1xBQ3XkKQ8CmjDfuAkx8E5cWB3g"
...
def browse():
    ...
    folders = []
    try:
        items   = client.storage.from_(STORAGE_BUCKET).list("")
        folders = sorted([i["name"] for i in items if i.get("id") is None])
    except Exception:
        pass
    ...
    try:
        if active_folder:
            items = client.storage.from_(STORAGE_BUCKET).list(active_folder)
            all_files = [(active_folder, i["name"]) for i in items if i["name"].lower().endswith(".pdf")]
        else:
            for folder in folders:
                items = client.storage.from_(STORAGE_BUCKET).list(folder)
                for i in items:
                    if i["name"].lower().endswith(".pdf"):
...
def download():
    ...
    try:
        pdf_bytes          = client.storage.from_(STORAGE_BUCKET).download(f"{folder}/{filename}")
        pdf_out, code, uid = stamp_pdf(pdf_bytes)

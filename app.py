...
def browse():
    folders = []
    try:
        items   = supabase.storage.from_(STORAGE_BUCKET).list("FORMS")
    ...
    try:
        if active_folder:
            items = supabase.storage.from_(STORAGE_BUCKET).list(f"FORMS/{active_folder}")
            all_files = [(active_folder, i["name"]) for i in items if i["name"].lower().endswith(allowed)]
        else:
            for folder in folders:
                items = supabase.storage.from_(STORAGE_BUCKET).list(f"FORMS/{folder}")
                for i in items:
...
def download():
    ...
    try:
        file_bytes = supabase.storage.from_(STORAGE_BUCKET).download(f"FORMS/{folder}/{filename}")
...
def debug():
    ...
    try:
        ...
        result["root"] = [{"name": i["name"], "id": i.get("id"), "metadata": i.get("metadata")} for i in root]
    except Exception as e:
        result["root_error"] = str(e)
    try:
        forms = supabase.storage.from_(STORAGE_BUCKET).list("FORMS")
        result["FORMS"] = [{"name": i["name"], "id": i.get("id"), "metadata": i.get("metadata")} for i in forms]
    except Exception as e:

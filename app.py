...
SUPER_ADMIN_EMAIL = "jmr.tibon@clcqea.com"
...
def super_admin_required(f):
    ...
    def decorated(*args, **kwargs):
        if not session.get("logged_in"):
            return redirect(url_for("login"))
        if session.get("user_email") != SUPER_ADMIN_EMAIL:
            return redirect(url_for("browse"))
        return f(*args, **kwargs)
    return decorated
...
def get_auth_client():
    """Returns a Supabase client authenticated with the current session's access token."""
...
def get_admin_emails():
    """Returns a set of all admin emails from the admins table."""
    try:
        res = supabase.table("admins").select("email").execute()
        return set(r["email"] for r in res.data)
    except Exception:
    ...
    if request.method == "POST":
        ...
        if role == "admin":
            ...
            if email not in admin_emails:
                ...
                # Promoted admin login via Supabase auth
                try:
                    res = supabase.auth.sign_in_with_password({"email": email, "password": password})
                    session["logged_in"]      = True
                    session["access_token"]   = res.session.access_token
...
def browse():
    ...
    return render_template("browse.html",
        ...
        is_super_admin=session.get("user_email") == SUPER_ADMIN_EMAIL,
...
def admin_upload():
    ...
    try:
        ...
        auth_client.storage.from_(STORAGE_BUCKET).upload(path, file_bytes, {"content-type": mime})
...
def admin_delete():
    ...
    try:
        auth_client = get_auth_client()
        auth_client.storage.from_(STORAGE_BUCKET).remove([f"{FORMS_FOLDER}/{folder}/{filename}"])
...
def admin_rename():
    ...
    try:
        ...
        auth_client = get_auth_client()
        file_bytes  = auth_client.storage.from_(STORAGE_BUCKET).download(old_path)
        auth_client.storage.from_(STORAGE_BUCKET).upload(new_path, file_bytes, {"content-type": mime})
        auth_client.storage.from_(STORAGE_BUCKET).remove([old_path])
...
def manage_users():
    """Returns all registered users and which ones are admins."""
    try:
        # Get all users from Supabase Auth (admin API)
        res   = supabase.auth.admin.list_users()
        users = [{"email": u.email, "id": str(u.id)} for u in res if u.email]
    except Exception:
        users = []
    ...
    for u in users:
        ...
        u["is_super"] = u["email"] == SUPER_ADMIN_EMAIL
...
def promote_user():
    ...
    try:
        supabase.table("admins").insert({"email": email}).execute()
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/admin/users/demote", methods=["POST"])
@super_admin_required
def demote_user():
    ...
    email = data.get("email", "").strip()
    if not email or email == SUPER_ADMIN_EMAIL:
        return jsonify({"error": "Cannot demote super admin"}), 400
    try:

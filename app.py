...
def debug():
    ...
    try:
        ...
        result["root"] = [{"name": i["name"], "metadata": str(i.get("metadata"))} for i in root]
    except Exception as e:
        result["root_error"] = str(e)
    try:
        forms = supabase.storage.from_(STORAGE_BUCKET).list(FORMS_FOLDER)
        result["FORMS_folder"] = [{"name": i["name"], "metadata": str(i.get("metadata"))} for i in forms]
    except Exception as e:
        result["FORMS_folder_error"] = str(e)
...
def download():
    ...
    try:
        file_bytes = supabase.storage.from_(STORAGE_BUCKET).download(f"{FORMS_FOLDER}/{folder}/{filename}")
        if ext == ".pdf":
            file_out, code, uid = stamp_pdf(file_bytes)
            new_name = f"{os.path.splitext(filename)[0]} - {uid}.pdf"
            mimetype = "application/pdf"
        elif ext == ".docx":
            uid, code = generate_clc_id()
            file_out  = BytesIO(file_bytes)
            new_name  = f"{os.path.splitext(filename)[0]} - {uid}.docx"

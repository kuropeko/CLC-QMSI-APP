...
@login_required
def chat():
    # Update heartbeat so user shows as online
    _heartbeat()
...
@login_required
def chat_messages():
    """Return latest 100 messages as JSON (polled by browser)."""
    _heartbeat()
    try:
...
def chat_delete(msg_id):
    """Admin-only: delete a chat message."""
    try:
...
@login_required
def chat_online():
    """Return list of online users (heartbeat within last 2 minutes)."""
    _heartbeat()
...
def _heartbeat():
    """Upsert current user's last_seen timestamp."""
